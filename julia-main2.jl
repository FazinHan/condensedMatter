...
function conductivity(L=L, eta_factor=eta_factor, u=u, l0=l0)
    ...
    for (i, (kx1, ky1)) in enumerate(k_cartesian)
        for (j, (kx2, ky2)) in enumerate(k_cartesian)
            ...
            ky = ky1 - ky2
            kk = sqrt(kx^2 + ky^2)
            for I in 1:N_i*floor(Int, L)
                potential[i, j] += exp(1im * (kx * rand(-L/2:L/2) + ky * rand(-L/2:L/2))) * pot_function(kk, u, l0)
            end
        end
    end
...
function main(L, rank)
    ...
    conductivities = [conductivity(l, eta_factor, u, l0) for l in L]
